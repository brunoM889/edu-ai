...
export async function POST(req) {
  ...
  const modalidad = await res.modalidad;
  let fragmentos = [];
  const apunteText = res.apunte;
  let tipoDeProceso = 0;

  let numpages = 1;
  console.log(apunteText.length);
  if (apunteText.length >= 1000) {
    numpages = apunteText.length / 1000;
  }
  ...
  if (numpages > 100) {
    ...
    tipoDeProceso = 10;
    const chars = apunteText.length / tipoDeProceso;
    for (let i = 0; i < tipoDeProceso; i++) {
      fragmentos.push(apunteText.slice(i * chars, chars + i * chars));
    }
  } else if (numpages >= 30) {
    tipoDeProceso = 5;
    const chars = apunteText.length / tipoDeProceso;
      ...
      if (modalidad == "Flash cards") {
        ...
        let prompt = `
          ...
          ACLARACIONES:
            ...
            ***MUY IMPORTANTE***
            Si el texto que recibes no parece ser sobre temas academicos, vas a responder con un $.
            En cambio si el pdf es valido, vas a devolver 10 preguntas/consignas  pura y exclusivamente en el material del pdf que **pueden ser respondidas en formato multiple choice, es decir, las respuestas a estas preguntas van a ser relativamente cortas** y debajo de cada pregunta 4 respuestas posibles, la respuesta correcta si o si va a estar marcada con un "*" al principio, y el resto deben ser INCORRECTAS pero (EL HECHO DE QUE SEA INCORRECTA NO SIGNIFICA QUE DEBAS RESPONDER ALGO ABSURDO). Por ejemplo, supongamos que el pdf trata sobre "analisis matematico, limites" la respuesta deberia tener exactamente el siguiente formato, ni un renglon mas ni uno menos (OBVIAMENTE LAS CONSIGNAS QUE TIENES QUE DEVOLVER DEBEN ESTAR BASADAS EN EL MATERIAL DEL PDF, ESTE ES SOLO UN EJEMPLO, NO DEBES DEVOLVER NADA RELACIONADO AL SIGUIENTE EJEMPLO):
